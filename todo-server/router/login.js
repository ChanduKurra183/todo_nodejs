...
login.post("/login", async (req, res) => {
    ...
    if (!user) return res.status(404).send({ status: false, message: "User does'nt exists." });

    const isMatch = await bycrypt.compare(password, user.password);
    console.log('ismatched', isMatch);

    if (!isMatch) return res.status(406).send({ status: false, message: "Username/Password not correct." })
...
login.post("/otp", async (req, res) => {
    ...
    if (!(email && password)) return res.status(406).send({ status: false, message: 'email-id/password required.' });
    ...
    try {
        ...
        if(!isMatch) return res.status(406).send({status: false, message: 'email/password In-correct'});
        // generating 4 digit opt number.
        otp = generateOtp();
        // save otp with user email-id.
        await Otp.findOneAndUpdate({ email: email }, { email: email, otp: otp }, { upsert: true });
        // send OTP to user email.
        EmailServices.sendEmail(email, FROM, SUBJECT, `OTP: ${otp}`);

        res.status(200).send({status:true, message: 'OTP generation successfull.'});

    } catch (error) {
        console.log('Error in OTP generation');
        console.log(error);

        res.status(406).send({status: false, message: 'OTP generation failed.'})
    }
...
login.post("/verify", async (req, res) => {
    ...
    if (!email) return res.status(406).send({ status: false, message: 'email-id required.' });
    ...
    try {
        ...
        if (!user) return res.status(404).send({ status: false, messgae: 'OTP is not generated for the user. please try loggin In.' });
        // send 406 error message if otp is incorrect.
        if(user.otp !== otp) return res.status(406).send({status: false, message: 'OTP In-correct.'});
        // delete otp generated record from database after verification.
        await Otp.deleteOne({email:email});

        res.status(200).send({status: true, message: 'OTP verification successfull.'});
        
    } catch (error) {
        console.log('Error in OTP verification');
        console.log(error);

        res.status(406).send({status: false, message: 'OTP verification failed.'})
